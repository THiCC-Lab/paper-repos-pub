...
class Chunk():
    ...
    def __init__(self, chunk_args, chunk_name=None, 
                 ...
                 store_chunk=False, request=False):
...
class HDM():
    ...
    # Commented out bc has no clear purpose in ACT-R Lisp syntax. There is no "bound dictionary" and iirc dr dancy and I cou   
    # assignValues checks for unassigned values, i.e., '?stuff'
    # takes Chunk chunk object and modifies it
    ...
    #     # deepcopy so it's ok to change indices/vals in loop
    #     attributes = copy.deepcopy(chunk.to_list())
    ...
    #                         except:
    #                             value = '?'
    #                     # handle case where value is modified
    #                     if chunk.has_slots:
    #                         chunk.contents[slot] = value
    #                     else:
    #                         chunk.contents[i] = value
    ...
    #     # object modified - don't need to return
        ...
        if not slot_pull:
            ...
            for t in range(self.chunk_count):
                ...
                for (slot_name, mt) in self.time_mem.items():
                    ...
                    sim = time_hrr.compare(mt * query_vec_inv)
