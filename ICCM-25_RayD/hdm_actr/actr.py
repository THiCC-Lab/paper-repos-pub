...
# Added by me
# doesn't include sorted param bc not sure how boolean True (python) translates to boolean t (Lisp)
def chunk_filled_slots_list(chunk_name, ordered=True):
    return current_connection.evaluate_single("chunk-filled-slots-list", chunk_name, ordered)
...
def printed_chunk_spec(spec_id):
    return current_connection.evaluate_single("printed-chunk-spec", spec_id)

def chunk_spec_slot_spec(spec_id):
    return current_connection.evaluate_single("chunk-spec-slot-spec", spec_id)

def chunk_spec_slots(spec_id):
    return current_connection.evaluate_single("chunk-spec-slot-spec", spec_id)

def release_chunk_spec(spec_id, slot=None):
    if slot is not None:
        return current_connection.evaluate_single("release-chunk-spec-id", spec_id, slot)
