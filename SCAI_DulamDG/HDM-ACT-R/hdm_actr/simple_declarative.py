# Meera: Copied here for debugging convenience
...
class declarative_module():
    ...
    def __init__(self):
        ...
        print("creating a declarative module object")
...
def retrieved_chunk(name,chunk):
    print(f"retrieved_chunk called on {chunk}")
...
def request (name,buffer,spec):
    print(f"Requesting a chunk with spec {spec}: {actr.chunk_spec_to_chunk_def(spec)}")
    # print(f"Spec {spec} looks like: ", )
    module = current_module(name)
    ...
    print(f"The match for chunk is:", match)
...
def add_chunk_to_dm (module,chunk):
    print("Adding this chunk to DM:", chunk)
...
actr.call_command("extend-chunks","unknown")
