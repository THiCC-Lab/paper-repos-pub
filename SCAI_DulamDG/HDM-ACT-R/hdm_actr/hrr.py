...
class HRR:
    ...
    def setMemVec(self):
        with open("vectors/iat458_w_vectors.pkl","rb") as f:
             self.value_vector_dict = pickle.load(f) 
    ...
    def __pow__(self, other, modulo=None):
        # implements fractional binding (see Komer 2019)
        """other should be a real number scalar"""
        if modulo is not None or isinstance(other,HRR):
            raise Exception("fractional binding only defined for one HRR" 
                            + " exponentiated to a real number")
        x = ifft(fft(self.v) ** other).real
        return HRR(data=x)
    def __ipow__(self, other):
        if isinstance(other,HRR):
            raise Exception("fractional binding only defined for one HRR" 
                            + " exponentiated to a real number")
        self.v = ifft(fft(self.v) ** other).real
        return self
    def copy(self):
        return HRR(data=self.v)
    def mse(self,other):
    ...
    def __repr__(self):
        N = len(self.v)
        v_str = self.v if N < 10 else f"{self.v[:10]}..."
        return f"HRR(N={N}, v={v_str})"
