...
#Start the HTML File
to_html = ["<!DOCTYPE html>","<html>","<head>","<link rel='stylesheet' href='content_overload.css'/>",
           "<title>","Testing CCP","</title>","</head>"]

#Character Overload
character_amount = 10000
step = 0

while step < character_amount:
        to_html.append(''.join(['<p class="character">',chr(random.randint(ord('a'), ord('z'))),'</p>']))
        step += 1
        if step % 10 == 0:
             to_html.append('<p class="character">. </p>')
...
to_html.append(''.join(['<p class="letter">',body,'</p>']))

#End the HTML File
to_html.append("</html>")

#Save the HTML File
file = open('content_overload.html','w')
for i in to_html:
    file.write(str(i))
file.close()
